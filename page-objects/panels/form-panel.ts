...
export class FormPanel {
    ...
    static async getFormTitleList() {
        const titleCount = await FormPanel.FORM_TITLE_SELECTOR.count;
        const titleList = [];
        for (let i = 3; i < titleCount; i++) {
            const currentTitle = await FormPanel.FORM_TITLE_SELECTOR.nth(i).innerText;
            titleList.push(currentTitle)
        }
        return titleList;
    }
}